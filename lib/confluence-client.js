...
class ConfluenceClient {
  constructor(config) {
    ...
    this.cookies = config.cookies; // For web auth
    ...
    if (this.authType === 'web') {
      // Web auth uses cookies instead of Authorization header
      if (this.cookies) {
  ...
  async downloadAttachment(pageIdOrUrl, attachmentIdOrAttachment, options = {}) {
    ...
    if (this.authType === 'web') {
      if (this.cookies) {
        downloadHeaders['Cookie'] = this.cookies;
      }
    } else if (this.authType === 'basic') {
      downloadHeaders['Authorization'] = this.buildBasicAuthHeader();
    } else {
      downloadHeaders['Authorization'] = `Bearer ${this.token}`;
    }
