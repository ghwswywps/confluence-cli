...
const AUTH_CHOICES = [
  ...
  { name: 'Web browser (SSO/SAML)', value: 'web' }
];
...
function getConfig() {
  ...
  if (!fs.existsSync(CONFIG_FILE)) {
    ...
    console.log(chalk.yellow('Please run "confluence init" or "confluence login_web" to set up your configuration.'));
  ...
  try {
    ...
    const authType = storedConfig.authType || normalizeAuthType(storedConfig.authType, Boolean(trimmedEmail));
    let apiPath;

    // For web auth, token is not required in config (cookies are used)
    if (authType !== 'web' && !trimmedToken) {
    ...
    if (!trimmedDomain) {
      console.error(chalk.red('❌ Configuration file is missing domain.'));
      console.log(chalk.yellow('Run "confluence init" to refresh your settings.'));
      process.exit(1);
    }
    ...
    return {
      ...
      email: trimmedEmail,
      authType,
      cookies: storedConfig.cookies // For web auth
    };
  } catch (error) {
